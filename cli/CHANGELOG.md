...
- 修复：RP 酒馆冷启动不再依赖外置 `SillyTavernBridge`、AWS/LiteLLM 桥、Python 自动路由器或 PowerShell 脚本；Tran 先隐藏准备 npm 依赖，再直接启动 `node.exe server.js`，最多等待三分钟并在就绪后打开独立窗口，运行期间不再保留 cmd 父进程。
...
- Fix: RP Tavern cold start no longer depends on the external `SillyTavernBridge`, AWS/LiteLLM bridge, Python auto-router, or PowerShell scripts. Tran prepares npm dependencies in a hidden transient process, then launches `node.exe server.js` directly, waits up to three minutes, and opens its dedicated window after readiness without retaining a cmd parent process.
...
- 新增：鼠标悬停消息显示复制按钮。AI 回复支持两种格式——复制 Markdown 源文、复制渲染后的富文本；用户消息支持一键复制文本。
- 新增：skill 列表改由主进程直接扫描磁盘（`~/.agents/skills` 与项目 `.agents/skills` 下的 SKILL.md），进入会话前 `/` 菜单和技能页即可显示；会话建立后仍以 ACP 下发的列表为准。
...
- 修复:**预览卡里的归档按钮点不到**——行移出立即收卡，指针跨不过行与卡之间的空隙。改为 300ms 延迟收卡 + 指针上卡常开（hover 桥）；A 行快速移到 B 行时旧卡让位，不误杀新卡预览。
