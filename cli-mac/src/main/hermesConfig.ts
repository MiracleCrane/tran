...
export function hermesConfigPath(): string {
  ...
  // macOS: ~/.hermes/config.yaml (XDG-ish home location). LOCALAPPDATA is a
  // Windows env var and is not set on macOS.
  cachedConfigPath = join(homedir(), '.hermes', 'config.yaml')
  return cachedConfigPath
}
