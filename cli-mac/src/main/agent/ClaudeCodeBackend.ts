...
import { resolveMacClaudeCommand, spawnClaudeViaMacPath } from '../macClaude'
...
export class ClaudeCodeBackend {
  ...
  private async spawn(
    ...
    // macOS: resolve the `claude` binary ourselves and drive the spawn. A
    // GUI-launched Electron app does NOT inherit the user's shell PATH
    // (~/.zshrc modifications are lost), so the SDK's own PATH lookup would
    // miss homebrew/npm-installed Claude Code. Pointing it at the resolved
    // absolute path (plus our explicit spawn) is the robust path.
    const macClaude = resolveMacClaudeCommand(env)
    ...
    const options: any = {
      ...
      env,
      pathToClaudeCodeExecutable: macClaude.command,
      spawnClaudeCodeProcess: spawnClaudeViaMacPath,
