 ...
 * settings.json env. ANTHROPIC_BASE_URL, the active auth key, and the
 * provider's default model (ANTHROPIC_MODEL) are synced; MCP servers and other
...
function applyToClaudeConfig(p: Provider, backend: ProviderBackend): void {
  ...
  // Sync the provider's default model so Claude Code picks it up. A non-empty
  // model writes ANTHROPIC_MODEL; clearing it removes the override (falls back
  // to Claude's own default).
  if (p.model && p.model.trim()) {
    env['ANTHROPIC_MODEL'] = p.model.trim()
  } else {
