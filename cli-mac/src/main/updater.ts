...
/** Resolve the per-OS update asset name for a given version.
 *
 *  - Windows: the NSIS setup exe (Forge-X.Y.Z-external-claude-setup.exe).
 *  - macOS: a per-arch .dmg (Forge-X.Y.Z-external-claude-mac-arm64.dmg on
 *    Apple Silicon, ...-mac-x64.dmg on Intel). Matches electron-builder's
 *    artifactName for the mac build.
 */
function installerAssetForVersion(version: string, tag = `v${version}`): UpdateAssetInfo {
  let name: string
  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'
    name = `Forge-${version}-external-claude-mac-${arch}.dmg`
  } else {
    name = `Forge-${version}-external-claude-setup.exe`
  }
  return {
    name,
...
/** Derive a safe local filename for the downloaded asset, accepting either a
 *  Windows setup .exe or a macOS .dmg/.zip. */
function safeAssetName(assetUrl: string): string {
  const fallback =
    process.platform === 'darwin' ? 'Forge-update-mac.dmg' : 'Forge-update-setup.exe'
  try {
    const parsed = new URL(assetUrl)
    const name = decodeURIComponent(parsed.pathname.split('/').filter(Boolean).pop() ?? '')
    return /\.(dmg|zip|exe)$/i.test(name) ? name : fallback
  } catch {
    return fallback
  }
}
...
export async function downloadAndInstallUpdate(
  ...
  try {
    ...
    if (normalized.openWhenDone !== false) {
      if (process.platform === 'darwin' && /\.zip$/i.test(destination)) {
        // A .zip is not directly runnable; reveal it in Finder so the user can
        // expand it and drag the .app into Applications.
        shell.showItemInFolder(destination)
      } else {
        // Windows: launches the NSIS setup. macOS: opens the .dmg so it mounts
        // and the drag-to-Applications window appears.
