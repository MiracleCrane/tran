...
const NATIVE_LABEL = isMac ? 'macOS' : 'Windows'
...
function backendName(backend: ProviderBackend): string {
  ...
  return backend === 'wsl' ? 'WSL' : isMac ? 'macOS' : 'Windows'
}
...
export default function ProvidersPanel(): JSX.Element {
  ...
  const displayBackendName = isHermesProfile ? NATIVE_LABEL : backendName(editingBackend)
    ...
    <div className="h-full overflow-y-auto bg-bg-base">
      <div className="mx-auto max-w-3xl px-6 py-6">
        <ToolPanelHeader
          ...
          description={
            isHermesProfile
              ? `当前显示 ${NATIVE_LABEL} 本机 Hermes 运营商配置，来源于 Hermes config.yaml。`
              : wslSupportEnabled
              ? `${NATIVE_LABEL} 和 WSL 完全独立。当前正在编辑 ${displayBackendName} 运营商配置。`
              : `当前显示 ${NATIVE_LABEL} 运营商配置。`
          }
          actions={
            <>
              <ToolPanelButton
