...
function WindowTitlebar(): JSX.Element {
  // macOS uses the native traffic-light buttons (titleBarStyle 'hiddenInset' in
  // the main process), so we render only a drag region with left padding to make
  // room for them. Windows keeps its custom minimize/maximize/close controls.
  if (isMac) {
    return (
      <div className="window-titlebar window-titlebar-mac flex shrink-0 items-center">
        <div className="window-titlebar-drag h-full flex-1" />
      </div>
    )
  }
  return (
